...
import graph.event.VertexTraversalEvent;
   ...
   extends AbstractGraphIterator<V, E>
{
   ...
   public RandomWalkIterator(Graph<V, E> graph, V startVertex, boolean isWeighted, long maxSteps)
   {
       ...
       this.originalMaxSteps = maxSteps;
     ...
     * @param isWeighted set to <code>true</code> if a weighted walk is desired.
     * @param maxSteps number of steps before walk is exhausted.
     * @param minSteps min number of steps a walk must have
     *
     * @throws IllegalArgumentException if <code>graph==null</code> or does not contain
     *         <code>startVertex</code>
     */
    public RandomWalkIterator(Graph<V, E> graph, V startVertex, E edgeToTraverse, boolean isWeighted, long maxSteps, int minSteps)
    {
        ...
        if(graph.containsEdge(edgeToTraverse)){
            ...
            throw new IllegalArgumentException("graph must contain the edge to traverse");
        }
    }
   ...
   public RandomWalkIterator(Graph<V, E> graph, V startVertex, V vertexToCover, long maxSteps)
   {
	   ...
	   if(graph.containsVertex(vertexToCover)){
		   ...
		   throw new IllegalArgumentException("graph must contain the vertex to cover");
	   }
   }
   ...
   {
       if(this.edgeToTraverse != null && this.minSteps != -1){
           if(!this.targetReached && !isExhausted() && !sinkReached){
               return true;
           }
           if(this.originalMaxSteps - this.maxSteps < this.minSteps){
               return true;
           }
           return false;
       }
	   else if(this.edgeToTraverse != null){
   ...
   public V next()
   {
       ...
       if (nextEdge != null) {
           ...
           if(this.edgeToTraverse.equals(this.currentEdge)){
